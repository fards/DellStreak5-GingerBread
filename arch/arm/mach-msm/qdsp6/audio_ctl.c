...
atomic_t voice_started=ATOMIC_INIT(0);  					/* Jagan+ ... Jagan- */
...
static int q6_voice_start(void)
{
	...
	if (atomic_read(&voice_started)) {					/* Jagan+ ... Jagan- */
	...
	atomic_set(&voice_started,1);					/* Jagan+ ... Jagan- */
done:
...
static int q6_voice_stop(void)
{
	...
	if (atomic_read(&voice_started)) {				/* Jagan+ ... Jagan- */
		...
		atomic_set(&voice_started,0);				/* Jagan+ ... Jagan- */
	}
	mutex_unlock(&voice_lock);
	return 0;
}
