...
static void lsensor_get_lux(void)
{
  ...
  const unsigned int coef_3[3][2] = { {1059, 2395}, {1795, 4238}, {455, 887}};   // *1300 / 9.5
...
static void lsensor_update_equation_parameter(struct lsensor_cal_data *p_cal_data)
{
  ...
  else
  {
    ...
    else if((p_cal_data->prox.prox_far + 100) >= p_cal_data->prox.prox_near)  //far and near difference too small
    {
