...
struct touchpad_t {
	...
	int                      fvs_mode_flag;  
	uint8_t                  i2c_addr;
	int check_home_key; //check if home key is pressed
	int chk_count;
...
static int should_send_k(uint16_t *data)
{
	...
	uint16_t 	check_mask;
		...
		{
			for( j = 0; j < NUM_OF_Y_CH-1; j++ ) //count how many bits set for this row
			{
				check_mask = 1 << j; //create a bit mask to check against
...
static int check_mXT224_touch_calibration(struct touchpad_t *g_tp, int * send_k)
{
	...
	uint8_t 	try_ctr;
	uint8_t 	value;
	uint8_t 	num_of_tch; //number of touch nodes
	uint8_t 	num_of_atch; //number of antitouch nodes
	uint8_t 	CAL_THR;
	uint16_t 	check_mask;
	uint16_t	addr;
   //uint16_t   addr_T8;
	...
	*send_k = 0;
	...
	num_of_tch = 0;
	num_of_atch = 0;
	CAL_THR = 10; //calibration threshold
	if(g_tp->touch_suspended)
		return 0;
	printk("==============================\n");
	/* touch/antitouch mode */
	value = 0xF3; //diagnostic command to get touch flags
	...
	if (result)
	{
		MY_INFO_PRINTK( 2,"ERROR_LEVEL:" "failed to read T37 data!\n" );
        	result = -EFAULT;
		return result;
	}
	j = 2;
	...
	MY_INFO_PRINTK( 1,"INFO_LEVEL:" "Mode[0] : 0x%x\n",data[0] );
	...
	{	
		...
		for( i = 0 ; i < NUM_OF_X_CH; i++ )
		{
			...
			for( j = 0; j < NUM_OF_Y_CH; j++ ) //count how many bits set for this row
			{
				check_mask = 1 << j; //create a bit mask to check against
...
//if 1st home key is pressed ,then call chk_work irqworkhandler
